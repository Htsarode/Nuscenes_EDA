...
def plot_object_behaviour(behaviour_data, output_dir="figures/exploratory"):
    ...
    Args:
        behaviour_data: Dictionary with object behaviour counts
        output_dir: Directory to save the plots
    """
    os.makedirs(output_dir, exist_ok=True)

    # --- Ensure 2 fixed labels are always present ---
    expected_labels = ["Moving", "Parked"]
    ...
    colors = ['#ff9999', '#66b3ff']  # Red for moving, blue for parked
    ...
    stats_text = f"""Object Behaviour Distribution Summary

Total Frames: {total}
Behaviours: {len(labels)}
...
Distribution:"""
    ...
    for chart in selected_charts:
        ...
        fig.suptitle(f"{chart.capitalize()} Chart with Statistical Summary - Object Behaviour Distribution",
        ...
        if chart == "bar":
            ...
            ax_chart.set_title("Bar Chart - Object Behaviour Counts", fontsize=14, fontweight="bold")
            ...
            ax_chart.plot(angles, radar_values, "o-", linewidth=3, color='darkred', markersize=10)
            ax_chart.fill(angles, radar_values, alpha=0.25, color='lightcoral')
            ...
            ax_chart.set_title("Histogram - Frame Counts per Behaviour", fontsize=14, fontweight="bold")
            ...
            ax_chart.bar(labels, [v/2 for v in values], color="teal", label="50% Mark", alpha=0.6)
            ax_chart.set_title("Stacked Bar Chart - Object Behaviours", fontsize=14, fontweight="bold")
            ax_chart.set_xlabel("Object Behaviour", fontsize=12, fontweight="bold")
            ax_chart.set_ylabel("Frame Count", fontsize=12, fontweight="bold")
            ax_chart.legend()
            ax_chart.grid(True, alpha=0.3)
            ...
            ax_chart.scatter(x_positions, values, color="darkgreen", s=150, alpha=0.7, edgecolors='black')
            ax_chart.set_xticks(x_positions)
            ax_chart.set_xticklabels(labels)
            ax_chart.set_title("Scatter Plot - Object Behaviours", fontsize=14, fontweight="bold")
            ax_chart.set_xlabel("Object Behaviour", fontsize=12, fontweight="bold")
            ax_chart.set_ylabel("Frame Count", fontsize=12, fontweight="bold")
            ax_chart.grid(True, alpha=0.3)
        ...
        outpath = os.path.join(output_dir, f"object_behaviour_{chart}_with_stats.png")
...
if __name__ == "__main__":
    print("🎯 Object Behaviour EDA Plotting Tool")
    print("=" * 40)
