...
def plot_traffic_density_weather(data: Dict[str, int], title: str = "Traffic Density vs Weather Conditions Analysis", output_dir: str = "figures/exploratory"):
    ...
    Args:
        data: Dictionary containing weather conditions and traffic density counts
    ...
    all_labels = ["Sunny", "Rainy", "Snow", "Clear", "Foggy", "Overcast", "Sleet"]
    ...
    weather_colors = {
        ...
        "Sleet": '#4682B4'       # Steel Blue
    }
    colors = [weather_colors[label] for label in labels]
    ...
    if choice == 1:  # Bar Chart
        ...
        non_zero_colors = [weather_colors[label] for label in non_zero_labels]
        ...
        sns.heatmap(heat_data, annot=True, fmt='d', cmap='YlOrRd', 
                   xticklabels=labels, yticklabels=['Traffic Density'],
        ...
        for i, (label, value) in enumerate(zip(labels, values)):
            plt.bar(["Traffic Density vs Weather"], [value], bottom=bottom, 
        ...
        df = pd.DataFrame({
            'Weather Conditions': [label for label, count in zip(labels, values) for _ in range(count)],
            'Count': [1] * sum(values)
        })
        
        if not df.empty:
            sns.kdeplot(data=df, x='Weather Conditions', weights='Count', 
    ...
    filename = f"traffic_density_weather_{chart_name}.png"
    ...
    # Print statistics
    total = sum(values)
    print(f"\n📊 Traffic Density vs Weather Conditions Statistics:")
    print(f"📈 Total traffic instances analyzed: {total}")
    
    if total > 0:
        for label, count in zip(labels, values):
            percentage = (count / total) * 100
            print(f"🔸 {label}: {count} traffic instances ({percentage:.1f}%)")
            
        # Additional insights
        print(f"\n💡 Key Insights:")
        max_weather = labels[values.index(max(values))]
        print(f"🔍 Highest traffic density weather: {max_weather} ({max(values)} instances)")
        
        # Weather categorization
        good_weather = ["Sunny", "Clear"]
        poor_weather = ["Rainy", "Snow", "Foggy", "Overcast", "Sleet"]
        
        good_traffic = sum(complete_data[weather] for weather in good_weather)
        poor_traffic = sum(complete_data[weather] for weather in poor_weather)
        
        print(f"☀️ Good weather traffic: {good_traffic} ({good_traffic/total*100:.1f}%)")
        print(f"🌧️ Poor weather traffic: {poor_traffic} ({poor_traffic/total*100:.1f}%)")
        
        if good_traffic > poor_traffic:
            print(f"📈 Traffic density is higher in good weather conditions")
        elif poor_traffic > good_traffic:
            print(f"📉 Traffic density is higher in poor weather conditions")
        else:
            print(f"⚖️ Traffic density is balanced across weather conditions")
    else:
        print("⚠️ No traffic density weather data found in the dataset")
...
if __name__ == "__main__":
    ...
    print("🔍 Run 'python main.py' and select the Traffic Density vs Weather Conditions Analysis option.")
