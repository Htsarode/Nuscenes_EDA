...
def plot_speed_bin(speed_data, output_dir="figures/exploratory"):
    """
    Plot speed bin (Low, Medium, High) frame counts with user-selected chart type.
    Args:
        speed_data: dict with keys 'Low Speed', 'Medium Speed', 'High Speed'
    ...
    stats_text = f"""Speed Bin Frame Count Summary\n\nTotal Frames: {total}\n"""
    ...
    for chart in selected_charts:
        ...
        fig.suptitle(f"{chart.capitalize()} Chart with Statistical Summary - Speed Bin Distribution",
        ...
        if chart == "bar":
            ...
            ax_chart.set_title("Bar Chart - Speed Bin Frame Counts", fontsize=14, fontweight="bold")
            ax_chart.set_xlabel("Speed Bin", fontsize=12, fontweight="bold")
            ax_chart.set_ylabel("Frame Count", fontsize=12, fontweight="bold")
            ax_chart.grid(True, alpha=0.3)
            add_custom_legend(ax_chart, "bar", "Number of frames in each speed bin", "Speed categories (Low/Medium/High)")
        elif chart == "pie":
            ax_chart.pie(values, labels=labels, autopct="%1.1f%%",
                         startangle=90, colors=colors, explode=[0.05]*len(labels))
            ax_chart.set_title("Pie Chart - Speed Bin Percentage Distribution", fontsize=14, fontweight="bold")
            add_custom_legend(ax_chart, "pie", "Frame count percentage in each speed bin", "Speed categories (Low/Medium/High)")
        elif chart == "donut":
            ...
            add_custom_legend(ax_chart, "donut", "Frame counts in each speed category", "Speed categories (Low/Medium/High)")
            ax_chart.text(0, 0, f"Total\n{total}", ha="center", va="center",
                          fontsize=12, fontweight="bold", color="darkblue")
            ax_chart.set_title("Donut Chart - Speed Bin Counts", fontsize=14, fontweight="bold")
        elif chart == "heatmap":
            sns.heatmap(np.array([values]), annot=True, fmt="d", cmap="YlGnBu",
                        xticklabels=labels, yticklabels=["Frame Count"], ax=ax_chart,
                        cbar_kws={'label': 'Count'})
            ax_chart.set_title("Heat Map - Speed Bin Intensity", fontsize=14, fontweight="bold")
            add_custom_legend(ax_chart, "heatmap", "Frame count intensity scale", "Speed categories (Low/Medium/High)")
        elif chart == "radar":
            fig.clf()
            fig, axes = plt.subplots(1, 2, figsize=(16, 7))
            ...
            ax_chart.set_title("Radar Chart - Speed Bin Distribution", fontsize=14, fontweight="bold")
            add_custom_legend(ax_chart, "radar", "Frame count radial distribution", "Speed categories (Low/Medium/High)")
        elif chart == "histogram":
            ax_chart.hist(values, bins=max(3, len(set(values))), color="skyblue", edgecolor="black", alpha=0.7)
            ax_chart.set_title("Histogram - Frame Counts per Speed Bin", fontsize=14, fontweight="bold")
            ...
            ax_chart.set_title("Stacked Bar Chart - Speed Bins", fontsize=14, fontweight="bold")
            ax_chart.set_xlabel("Speed Bin", fontsize=12, fontweight="bold")
            ax_chart.set_ylabel("Frame Count", fontsize=12, fontweight="bold")
            ax_chart.legend()
            ax_chart.grid(True, alpha=0.3)
            add_custom_legend(ax_chart, "stackedbar", "Total frames and 50% threshold", "Speed categories (Low/Medium/High)")
        elif chart == "scatter":
            x_positions = range(len(labels))
            ax_chart.scatter(x_positions, values, color="purple", s=150, alpha=0.7, edgecolors='black')
            ax_chart.set_xticks(x_positions)
            ax_chart.set_xticklabels(labels)
            ax_chart.set_title("Scatter Plot - Speed Bins", fontsize=14, fontweight="bold")
            ax_chart.set_xlabel("Speed Bin", fontsize=12, fontweight="bold")
            ax_chart.set_ylabel("Frame Count", fontsize=12, fontweight="bold")
            ax_chart.grid(True, alpha=0.3)
            add_custom_legend(ax_chart, "scatter", "Frame count for each speed category", "Speed categories (Low/Medium/High)")
        elif chart == "density":
            if len(set(values)) > 1:
        ...
        outpath = os.path.join(output_dir, f"speed_bin_{chart}_with_stats.png")
...
if __name__ == "__main__":
    print("🚗 Speed Bin EDA Plotting Tool")
    print("=" * 40)
