...
def plot_road_obstacles_distribution(road_obstacles, output_dir="figures/exploratory"):
    os.makedirs(output_dir, exist_ok=True)

    # Expected obstacles (force all labels present)
    expected_labels = ["Potholes", "Debris", "Closures", "Construction Zones"]
    cleaned = {label: road_obstacles.get(label, 0) for label in expected_labels}
    ...
    stats_text = f"""Road Obstacles Statistical Summary

Total Frames: {total_frames}
Obstacle Types: {len(labels)}
...
Distribution:"""
    ...
    for chart in selected_charts:
        ...
        fig.suptitle(f"{chart.capitalize()} Chart with Statistical Summary - Road Obstacles",
                     fontsize=16, fontweight="bold", y=0.98)
        ax_chart = axes[0]

        if chart == "bar":
            bars = ax_chart.bar(labels, values, color=colors, edgecolor="black")
            ax_chart.set_title("Bar Chart - Road Obstacles")
            ax_chart.set_ylabel("Frame Count")
            for bar in bars:
                h = bar.get_height()
                ax_chart.text(bar.get_x()+bar.get_width()/2, h, f"{int(h)}",
                              ha="center", va="bottom", fontweight="bold")
            add_custom_legend(ax_chart, "bar", "Number of road frames with obstacles observed", "Road obstacle types")
            ...
            add_custom_legend(ax_chart, "pie", "Number of road frames with obstacles observed", "Road obstacle types")
            ...
            add_custom_legend(ax_chart, "donut", "Number of road frames with obstacles observed", "Road obstacle types")

        elif chart == "heatmap":
            sns.heatmap(np.array([values]), annot=True, fmt="d", cmap="Reds",
                        xticklabels=labels, yticklabels=["Frames"], ax=ax_chart)
            ax_chart.set_title("Heat Map - Obstacle Intensity")
            add_custom_legend(ax_chart, "heatmap", "Number of road frames with obstacles observed", "Road obstacle types")
            ...
            ax_chart.set_title("Radar Chart - Obstacle Distribution")
            add_custom_legend(ax_chart, "radar", "Number of road frames with obstacles observed", "Road obstacle types")

        elif chart == "histogram":
            ax_chart.hist(values, bins=6, color="skyblue", edgecolor="black")
            ax_chart.set_title("Histogram - Frame Counts")
            add_custom_legend(ax_chart, "histogram", "Number of road frames with obstacles observed", "Road obstacle types")
            ...
            ax_chart.legend()
            add_custom_legend(ax_chart, "stackedbar", "Number of road frames with obstacles observed", "Road obstacle types")

        elif chart == "scatter":
            ax_chart.scatter(labels, values, color="purple", s=100)
            ax_chart.set_title("Scatter Plot - Obstacles")
            add_custom_legend(ax_chart, "scatter", "Number of road frames with obstacles observed", "Road obstacle types")

        elif chart == "density":
            sns.kdeplot(values, fill=True, color="green", ax=ax_chart)
            ax_chart.set_title("Density Plot - Frame Distribution")
            add_custom_legend(ax_chart, "density", "Number of road frames with obstacles observed", "Road obstacle types")

        add_stats(axes[1])

        plt.tight_layout(rect=[0, 0, 1, 0.95])
        outpath = os.path.join(output_dir, f"roadobstacles_{chart}_with_stats.png")
